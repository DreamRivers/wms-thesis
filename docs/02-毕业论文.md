...
本章对本系统所采用的主要理论与技术进行介绍,涵盖软件架构、数据库、接口、权限模型四大设计理论,以及 Spring Boot、MyBatis-Plus、Sa-Token、Vue 3、Element Plus、MySQL、Redis 等具体技术框架,为后续系统设计与实现提供理论与技术支撑。
...
### 2.8  前端框架 (Vue 3 + Element Plus + 前后端分离)


### 2.9  MySQL + Redis 数据存储
...
整体上，本系统数据库围绕"人—货—场—单据"四条主线进行表结构组织，共设计 21 张核心数据表，按业务领域划分为七大模块：

**（1）系统管理模块（6 张）**：包括 `sys_user`（系统用户表）、`sys_role`（系统角色表）、`sys_menu`（菜单权限表）、`sys_user_role`（用户—角色关联表）、`sys_role_menu`（角色—菜单关联表）以及 `sys_operation_log`（系统操作日志表）。该模块采用经典的 RBAC 三表+两关联结构：用户与角色之间、角色与菜单之间均为多对多关系，借助两张关联表解耦，便于权限的灵活配置与扩展。`sys_operation_log` 由自定义 `@Log` 注解 + AOP 切面统一写入，记录操作人、模块、操作类型、请求参数、响应结果、耗时及 IP 等信息，为系统审计与问题定位提供数据支撑。

**（2）主数据模块（5 张）**：包括 `wms_category`（商品分类表）、`wms_warehouse`（仓库表）、`wms_location`（库位表）、`wms_supplier`（供应商表）以及 `wms_goods`（商品表）。该模块是业务流转的基础：`wms_category` 与 `wms_goods` 构成一对多关系，商品分类按层级编码管理；`wms_warehouse` 与 `wms_location` 构成一对多关系，库位以"仓库—区域—货架—货位"四级编码唯一定位；`wms_supplier` 作为入库环节的来源方，与入库单关联，便于供应商评价与追溯。所有主数据表均设置 `status`（启用/停用）字段，支持业务上的软停用。

**（3）入库管理模块（2 张）**：`wms_inbound_order`（入库单主表）与 `wms_inbound_order_item`（入库单明细表）构成"主表+明细表"结构。主表存储单据编号、供应商、入库仓库、操作人、单据状态等维度信息；明细表存储具体商品的 SKU、批次号、计划数量、实际数量、库位等业务字段。同一商品在不同批次、不同库位下分别记录明细，便于批次管理与质量追溯。

**（4）出库管理模块（3 张）**：`wms_outbound_order`（出库单主表）、`wms_outbound_order_item`（出库单明细表）以及 `wms_outbound_approval`（出库审批流表）。出库模块在主—明细结构之外单独抽取审批流表，记录每一级审批的审批人、审批动作（同意/驳回）、审批意见、审批时间，实现审批过程的可追溯与可回放，避免审批信息散落在主表字段中导致的历史状态丢失问题。

**（5）库存管理模块（2 张）**：`wms_stock`（实时库存表）与 `wms_stock_record`（库存流水表）。实时库存表按 `(goods_id, location_id, batch_no)` 建立联合唯一索引，保证同一商品在同一库位同一批次下仅有一条库存记录；流水表按时间顺序记录每一次入库、出库、调整、盘点的数量变化，作为库存余额的"事实表"，二者结合既支持快速查询当前余额，又保留完整的变动轨迹。

**（6）盘点管理模块（2 张）**：`wms_stocktaking_order`（盘点单主表）与 `wms_stocktaking_order_item`（盘点明细表）。盘点单通过明细表记录每个 SKU 在每个库位的账面数量、实盘数量、差异数量及差异原因，盘点完成后自动生成差异调整单回写到库存流水，实现"账—实"差异的闭环处理。

**（7）通知模块（1 张）**：`wms_notification`（系统通知表），统一存储库存预警、临期提醒、审批待办等系统消息，支持用户维度的已读/未读管理。

在**通用规范**上，所有主键均采用 MySQL 自增 `id`（`bigint` 类型）以保证全局唯一与扩展性；主要业务表统一引入 `create_by / create_time / update_by / update_time / deleted` 五个公共字段，由 MyBatis-Plus 的 `MetaObjectHandler` 与 `@TableLogic` 注解自动填充与过滤，实现审计追踪与逻辑删除的标准化。所有业务表采用 `InnoDB` 存储引擎与 `utf8mb4` 字符集，统一支持事务与表情符号存储；为高频查询字段（如 `goods.sku_code`、`inbound_order.order_no`、`outbound_order.order_no`、`stock.goods_id` 等）建立单列索引或联合索引；为 `create_time`、`deleted` 等常用过滤字段建立索引以提升分页查询效率。在命名上，所有表统一采用"模块前缀 + 业务名称"的蛇形命名（如 `wms_inbound_order_item`），所有字段采用小写蛇形命名（如 `order_no`、`batch_no`），确保命名风格一致、可读性高。

在**关联关系**上，系统以 `wms_goods`、`wms_location`、`wms_stock` 为核心枢纽：商品经入库单明细写入库存，库存由出库单明细扣减，盘点明细与库存逐项核对；用户经角色与菜单权限关联，审批操作经 `wms_outbound_approval` 留痕，操作行为经 `sys_operation_log` 留痕，从而在数据层面形成"业务流—权限流—审计流"三流合一的闭环结构，为整个仓储管理系统的高效运转与可追溯性提供坚实的数据基础。
